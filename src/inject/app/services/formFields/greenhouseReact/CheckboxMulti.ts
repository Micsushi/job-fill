...
export class CheckboxMulti extends GreenhouseReactBaseInput {
  ...
  get labelElement(): HTMLElement {
    return getElement(this.element, `.//legend[starts-with(@class, "label")]`);
  }
  ...
  get choiceElements(): CheckboxWrapperContainer[] {
    return getElements(this.element, `.//div[@class="checkbox__wrapper"]`).map(
      (el) => new CheckboxWrapperContainer(el)
    );
  }
  ...
  async fill(answers: StringAnswerDTO[]): Promise<void> {
    await fieldFillerQueue.enqueue(async () => {
      ...
      for (const storedAnswer of answers) {
        const correctChoice = this.choiceElements.find(
          (el) => el.value === storedAnswer.answer
        );
        if (correctChoice) {
          correctChoice.check();
          break;
