...
export class File extends GreenhouseReactBaseInput {
  ...
  currentValue() {
    return (
      getElement(this.element, `.//div[@class="file-upload__filename"]`)
        ?.innerText || ''
    );
  }

  get deleteButtonElement(): HTMLElement {
    return getElement(this.element, ".//button[@aria-label='Remove file']");
  }
  ...
  async fill(answers: SingleFileAnswerDTO[]): Promise<void> {
